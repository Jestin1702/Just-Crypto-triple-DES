...
class Gui(threading.Thread):
    ...
    def run(self):
        ...
        self.win = Tk()
        self.win.iconbitmap('png.ico')
        self.win.config(cursor='dot')
        self.win.title('Just Crypto')
        self.win.geometry("600x500")
        self.win.protocol("WM_DELETE_WINDOW", self.exit_callback)
        bg = PhotoImage(file="maker1.gif")

        # Show image using label
        label1 = Label(self.win, image=bg)
        label1.place(x=0, y=0)

        # Set all the UI elements and the callback methods
        self.l1 = Label(self.win, text='Just Crypto', font=("Algerian", 30, "bold"),fg="red",anchor=NW)
        self.l2 = Label(self.win, text='Choose speed',font=15,anchor=NW)
        self.cb = Combobox(self.win, values=['Slow (Low CPU usage)', 'Medium (default)', 'Fast (High CPU usage)'], state='readonly')
        self.l3 = Label(self.win, text='Select the file to encrypt/decrypt', font=15)
        #self.t1 = Entry(print(), width=30)
        self.b1 = Button(self.win, text='Browse', command=self.browse, bg="orange", fg="red")
        self.l4 = Label(self.win, text='Enter the password', font=15)
        self.t2 = Entry(show='*')
        self.l5 = Label(self.win, text='Name of encrypted/decrypted file with EXT',font=15)
        self.t3 = Entry()
        self.b2 = Button(self.win, text='Encrypt', command=self.encrypt_pressed, bg="black", fg="red")
        self.b3 = Button(self.win, text='Decrypt', command=self.decrypt_pressed, bg="black", fg="red")
        self.l6 = Label(self.win, text='', font=("Ariel", 9, "bold"))

        self.l1.pack(pady = 20)
        self.l2.place(x=50, y=90)
        self.cb.place(x=50, y=120)
        self.l3.place(x=50, y=150)
        self.b1.place(x=50, y=200)
        self.b1.configure(borderwidth='2')
        #self.t1.place()
        self.l4.place(x=50, y=250)
        self.t2.place(x=50, y=290)
        self.l5.place(x=50, y=320)
        self.t3.place(x=50, y=350)
        self.b2.place(x=50, y=385)
        self.b3.place(x=150, y=385)
        self.l6.place(x=30, y=410)
        self.l6.configure(foreground='green')
        self.l6.config(text='')

        # Button for exit to be initialized after the process is over
        self.b4 = Button(self.win, text='Exit', command=self.exit_callback)
        self.b4.place(x=150, y=750)
        self.b4.destroy()
        # self.b4.grid_forget()
    ...
    def get_data(self):
        self.speed_sel()
        self.password = self.t2.get()
        self.out_name = self.t3.get()
...
if __name__ == '__main__':
    ...
    while gui_window.flag == 0:
        # Keep checking the UI thread every second to see if the user has pressed the "Encrypt" or "Decrypt" button
        time.sleep(1)
    ...
    gui_window.b4.place(x=150, y=750)
